...
class Configuration implements Singleton {
	...
	private function load_host_settings() {
		...
		if ($config_file) {
			...
			// TODO: Redirect to a setup script instead of triggering an error
			$this->set_error('critical',"Unable to locate configuration file for host \"".$this->host_name()."\".");
		}
	}
	/**
	...
	private function ensure_directory_setup() {
		...
		if (!Crumbs::ensure_directory(SITE_ROOT."/page_cache")) {
			$this->set_error('critical',"Page cache directory (/page_cache) does not exist or is not writable. This directory is required for Biscuit to operate.");
		}
		if (!Crumbs::ensure_directory(SITE_ROOT.'/css/cache')) {
			$this->set_error('critical',"CSS cache directory does not exist or is not writable! Program cannot continue. This directory is required for Biscuit to operate.");
		}
		if (!Crumbs::ensure_directory(SITE_ROOT."/js/cache")) {
			$this->set_error('critical',"Javascript cache directory does not exist or is not writable! Program cannot continue. This directory is required for Biscuit to operate.");
		}
	}
	/**
	...
	public function define_system_settings() {
		...
		$settings = DB::fetch("SELECT `constant_name`, `value` FROM `system_settings`");
		for ($i=0;$i < count($settings);$i++) {
			define($settings[$i]['constant_name'], $settings[$i]['value']);
		}
	}
	/**
