...
class Configuration implements Singleton {
	...
	private function load_host_settings() {
		...
		if ($config_file) {
			...
			// TODO: Redirect to a setup script instead of displaying an error message
			print "Unable to find a configuration file for ".$this->host_name().". Please create a configuration file to continue. To do this, copy the default configuration file (config.php) from /framework/config/default into /config/".$this->host_name()." and modify it from there.";
	...
	private function ensure_directory_setup() {
		if (!Crumbs::ensure_directory(SITE_ROOT."/page_cache")) {
			$this->set_error('warning',"Page cache directory (/page_cache) does not exist or is not writable. Server-side page caching will not occur.");
		}
		if (!Crumbs::ensure_directory(SITE_ROOT."/log")) {
			$this->set_error('warning',"Log directory (/log) does not exist or is not writable. All messages will be routed to the default PHP log file.");
		}
		if (!Crumbs::ensure_directory(SITE_ROOT."/uploads")) {
			$this->set_error('warning',"Uploads directory (/uploads) does not exist or is not writable. File uploads will fail.");
		}
		if (!Crumbs::ensure_directory(TEMP_DIR)) {
			$this->set_error('warning',"Temporary directory (/tmp) does not exist or is not writable. Certain types of file uploads may fail, such as images.");
		}
		if (!is_writable(FW_ROOT.'/extensions/html_purify/vendor/htmlpurifier/library/HTMLPurifier/DefinitionCache/Serializer')) {
			$this->set_error('warning',"HTML Purifier cache directory (/extensions/html_purify/vendor/htmlpurifier/library/HTMLPurifier/DefinitionCache/Serializer) is not writable.");
		}
		if (!Crumbs::ensure_directory(SITE_ROOT.'/css/cache')) {
			$this->set_error('critical',"CSS cache directory (/css/cache) does not exist or is not writable! Program cannot continue. This directory is required for Biscuit to operate.");
		}
		if (!Crumbs::ensure_directory(SITE_ROOT."/js/cache")) {
			$this->set_error('critical',"Javascript cache directory (/js/cache) does not exist or is not writable! Program cannot continue. This directory is required for Biscuit to operate.");
		}
	}
	/**
	...
	public function define_system_settings() {
		...
		$settings_factory = new ModelFactory('SystemSettings');
		$settings = $settings_factory->find_all();
