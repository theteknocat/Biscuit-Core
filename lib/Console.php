<?php
require_once('lib/vendor/FirePHPCore/fb.php');
/**
...
class Console {
	...
	public static function set_err_handler() {
		set_exception_handler(array('Console','handle_uncaught_exception'));
	...
	public static function log_dump($log_type) {
		if (DEBUG && SERVER_TYPE == "LOCAL_DEV") {	// Only output log if debugging mode enabled AND this is the local development machine.
			$log_file = SITE_ROOT."/log/".$log_type."_log";
			if (!file_exists($log_file)) {
	...
	public static function parse_log_file($type) {
		$log_file = SITE_ROOT.'/log/'.$type."_log";
		if (!file_exists($log_file)) {
			return array();
		} else {
			// Fetch log contents:
	...
	public static function throw_500($error_info) {
		...
		$report_sent = false;
		$backtrace = debug_backtrace();
		// Bump the first 2 elements of the backtrace array because they are always this method and the trigger_error function
...
Error Message: ".((!empty($error_info['errstr'])) ? $error_info['errstr'] : "None supplied.")."
...
".print_r($backtrace,true);
		...
		$html_message = <<<HTML
<p><strong>File:</strong> {$error_info['errfile']} | <strong>Line:</strong> {$error_info['errline']}</p>
{$error_info['errstr']}
HTML;
		$error_output = array(
	 ...
	 * @author Peter Epp
	 */
	public static function send_error_report($message,$subject = "Error Report") {
		if (defined("TECH_EMAIL")) {
	 ...
	 * @author Peter Epp
	 */
	public static function handle_uncaught_exception($e) {
