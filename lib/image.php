...
class Image {
	...
	public function __construct($full_file_path) {
		if (!$this->can_load_into_memory($full_file_path)) {
			$this->error(__("The image is too large to load into the available memory."));
			return;
		}
		...
		if ($this->image_is_valid()) {
			...
			$this->error(__("The image does not appear to be a valid JPEG, PNG or GIF. Maybe the file is corrupted?"));
		}
	}
	/**
