...
class ModuleCore extends ExtensionCore {
	...
	protected function install_or_uninstall_module($mode,$module_name) {
		...
		if ($this->ModuleAuthenticator()->user_is_super()) {
			...
			if (!empty($module_id)) {
				$this->load_controller($module_name);
		...
		if (!empty($page_modules)) {
			...
			foreach ($page_modules as $module) {
				...
				if (Crumbs::public_method_exists($this->modules[$module_name], "set_params")) {
					$this->modules[$module_name]->set_params($this->user_input);
				}
	...
	/**
	 * Return the date of the most recently updated module on the current page. Checks all database tables and files used by the module, returning the most recent
	 *
	 * @return void
	 * @author Peter Epp
	 */
	protected function latest_module_update() {
		$timestamps = array();
	...
	protected function run_page_modules() {
		if (empty($this->user_input['action'])) {
			$action = "index";
		}
		else {
