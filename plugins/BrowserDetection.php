...
Original Script Source URI: http://techpatterns.com/downloads/php_browser_detection.php
Version 4.9.8
...
- Conversion to object-based plugin for compatibility with Titanium template system
- Removal of requirment for $which_test parameter.  Instead all parameters get stored in an array

*******************************************/

// main script, uses two other functions, which_os() and browser_version() as needed
/**
 * Third party plugin written by Harold Hope (http://techpatterns.com) and adapted for use as a plugin with the Biscuit framework
 *
 * @package Plugins
 * @author Peter Epp
 **/
class BrowserDetection {
	...
	function run() {
		...
		*/
		if ( !$b_repeat )
		{
			...
			*/
			...
			$a_browser_types[] = array( 'opera', true, 'Opera', 'Browser' );
			$a_browser_types[] = array( 'omniweb', true, 'OmniWeb', 'Browser' );// mac osx browser, now uses khtml engine:
			$a_browser_types[] = array( 'msie', true, 'Internet Explorer', 'Browser' );
			$a_browser_types[] = array( 'konqueror', true, 'Konquerer', 'Browser' );
			$a_browser_types[] = array( 'safari', true, 'Safari', 'Browser' );
			// covers Netscape 6-7, K-Meleon, Most linux versions, uses moz array below
			$a_browser_types[] = array( 'gecko', true, 'Mozilla', 'Browser' );
			$a_browser_types[] = array( 'netpositive', false, 'NetPositive', 'Basic Browser' );// beos browser
			$a_browser_types[] = array( 'lynx', false, 'Lynx', 'Basic Browser' ); // command line browser
			$a_browser_types[] = array( 'elinks ', false, 'Elinks', 'Basic Browser' ); // new version of links
			$a_browser_types[] = array( 'elinks', false, 'Elinks', 'Basic Browser' ); // alternate id for it
			$a_browser_types[] = array( 'links ', false, 'Links', 'Basic Browser' ); // old name for links
			$a_browser_types[] = array( 'links', false, 'Links', 'Basic Browser' ); // alternate id for it
			$a_browser_types[] = array( 'w3m', false, 'w3m', 'Basic Browser' ); // open source browser, more features than lynx/links
			$a_browser_types[] = array( 'webtv', false, 'WebTV', 'Basic Browser' );// junk ms webtv
			$a_browser_types[] = array( 'amaya', false, 'Amaya', 'Basic Browser' );// w3c browser
			$a_browser_types[] = array( 'dillo', false, 'Dillo', 'Basic Browser' );// linux browser, basic table support
			$a_browser_types[] = array( 'ibrowse', false, 'IBrowse', 'Basic Browser' );// amiga browser
			$a_browser_types[] = array( 'icab', false, 'ICab', 'Browser' );// mac browser 
			$a_browser_types[] = array( 'crazy browser', true, 'Internet Explorer', 'Browser' );// uses ie rendering engine
			$a_browser_types[] = array( 'sonyericssonp800', false, 'SonyEricssonP800', 'Basic Browser' );// sony ericsson handheld
		
			// search engine spider bots:
			$a_browser_types[] = array( 'googlebot', false, 'Googlebot', 'bot' );// google 
			...
			$a_browser_types[] = array( 'yahoo-mm', false, 'Yahoo-MMCrawler', 'bot' ); // gets Yahoo-MMCrawler and Yahoo-MMAudVid bots
			$a_browser_types[] = array( 'inktomi', false, 'Inktomi', 'bot' ); // inktomi bot
			$a_browser_types[] = array( 'slurp', false, 'Inktomi', 'bot' ); // inktomi bot
			$a_browser_types[] = array( 'fast-webcrawler', false, 'Fast-Webcrawler', 'bot' );// Fast AllTheWeb
			$a_browser_types[] = array( 'msnbot', false, 'MSNBot', 'bot' );// msn search 
			$a_browser_types[] = array( 'ask jeeves', false, 'Ask Jeeves', 'bot' ); //jeeves/teoma
			$a_browser_types[] = array( 'teoma', false, 'Ask Jeeves', 'bot' );//jeeves teoma
			...
			$a_browser_types[] = array( 'ia_archiver', false, 'IA Archiver', 'bot' );// ia archiver
			$a_browser_types[] = array( 'zyborg', false, 'Looksmart', 'bot' );// looksmart 
			$a_browser_types[] = array( 'almaden', false, 'IBM Almaden Crawler', 'bot' );// ibm almaden web crawler 
			$a_browser_types[] = array( 'baiduspider', false, 'Baidu Spider', 'bot' );// Baiduspider asian search spider
			$a_browser_types[] = array( 'psbot', false, 'PSBot Image Crawler', 'bot' );// psbot image crawler 
			$a_browser_types[] = array( 'gigabot', false, 'GigaBot', 'bot' );// gigabot crawler 
			$a_browser_types[] = array( 'naverbot', false, 'Naverbot - BAD BOT', 'bot' );// naverbot crawler, bad bot, block
			...
			$a_browser_types[] = array( 'iltrovatore-setaccio', false, 'Iltrovatore-Setaccio', 'bot' );

			// various http utility libaries 
			$a_browser_types[] = array( 'w3c_validator', false, 'W3C Validator', 'lib' ); // uses libperl, make first
			$a_browser_types[] = array( 'wdg_validator', false, 'WDG Validator', 'lib' ); // 
			...
			// download apps 
			$a_browser_types[] = array( 'getright', false, 'Getright Downloader', 'dow' );
			$a_browser_types[] = array( 'wget', false, 'WGet Downloader', 'dow' );// open source downloader, obeys robots.txt

			// netscape 4 and earlier tests, put last so spiders don't get caught
			$a_browser_types[] = array( 'mozilla/4.', false, 'Netscape', 'Basic Browser' );
			$a_browser_types[] = array( 'mozilla/3.', false, 'Netscape', 'Basic Browser' );
			$a_browser_types[] = array( 'mozilla/2.', false, 'Netscape', 'Basic Browser' );
			...
			for ($i = 0; $i < count($a_browser_types); $i++)
			{
				...
				if (stristr($browser_user_agent, $s_browser)) 
				{
					...
					switch ( $browser_name )
					{
						...
						case 'Netscape':
							$safe_browser = false;
							$version_number = $this->browser_version( $browser_user_agent, 'mozilla' );
							break;
						case 'Mozilla':
							/*
						...
						*/
						...
						case 'OmniWeb':
							$s_browser = 'safari';
							$browser_name = 'Safari';
							$version_number = $this->browser_version( $browser_user_agent, 'applewebkit' );
							break; 
						*/
						default:
							$version_number = $this->browser_version( $browser_user_agent, $s_browser );
		...
		*/
		// Store all the browser info in an array:
		$this->info = array(
				'name' => $browser_name,
				'version' => $version_number,
				'numeric_version' => $math_version_number,
				'os_name' => $os,
				'windows_version' => $os_number,
				'ie_version' => $ie_version,
				'moz_version' => array($moz_version, $moz_version_number, $moz_rv, $moz_rv_full, $moz_release),
				'safe' => $safe_browser,
				'is_dom' => $dom_browser,
				'visitor_type' => $type,
				'user_agent' => $_SERVER['HTTP_USER_AGENT']);	// Stuff the original user agent string in there as well, as it might be handy for testing purposes
		/* The following was the original code that returned browser data based on the $which_test parameter.
		switch ( $which_test )
		{
	...
	function which_os ( $browser_string, $browser_name, $version_number  )
	{
		...
		*/
		...
		$a_os = array( 'BeOS', 'OS2', 'Amiga', 'WebTV', 'Mac', 'Windows NT', 'Windows', $a_unix, $a_linux );
			...
			if ( !is_array( $s_os ) && stristr( $browser_string, $s_os ) && !stristr( $browser_string, "linux" ) )
			{
				...
				switch ( $os )
				{
					...
					case 'mac':
						...
						elseif ( ( $browser_name == 'Safari' ) || ( $browser_name == 'Camino' ) || 
							( ( $browser_name == 'Mozilla' ) && ( $version_number >= 1.3 ) ) || 
							( ( $browser_name == 'Internet Explorer' ) && ( $version_number >= 5.2 ) ) )
						{
							$os_version = 10;
						}
						break;
