...
* Modified to run on PHP4 and adapted to integrate with the Biscuit framework by Peter Epp on September 8th, 2008
*
...
class SimpleCaptcha {
	...
	*/
	...
	/**
	 * Whether or not to use random text. Defaults to true.  Set to false to use dictionary text
	 *
	 * @var string
	 */
	var $useRandomText = true;

	/** GD image */
	var $im;

	function SimpleCaptcha($session_key) {
	...
	*/
	...
	function CreateImage() {
		...
		Session::set($this->session_key,$text);
	...
	*/
	function GetDictionaryCaptchaText($extended = false) {
		...
		$fp     = fopen($this->basedir.$this->wordsFile, "r");
		$length = strlen(fgets($fp));
		if (!$length) {
			return false;
		}
		$line   = rand(0, (filesize($this->basedir.$this->wordsFile)/$length)-1);
		...
		for ($i=0; $i<$length; $i++) {
			...
			if ($this->shadowColor) {
				$coords = imagettftext($this->im, $fontsize, $degree,
					...
					$this->GdShadowColor, $this->basedir.$fontfile, $letter);
			}
			$coords = imagettftext($this->im, $fontsize, $degree,
				$x, $y,
				$this->GdFgColor, $this->basedir.$fontfile, $letter);
